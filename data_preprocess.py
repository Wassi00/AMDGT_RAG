...
def k_fold(data, args):
    ...
    for i in range(k):
        fold_dir = args.data_dir + 'fold/' + str(i)
...
def cold_start_k_fold(data, args, mode):
    k = args.k_fold
    X = data['all_drdi']
    Y = data['all_label']

    if mode == 'cold_start_drug':
        entities = np.arange(args.drug_number)
        entity_index = 0
    elif mode == 'cold_start_disease':
        entities = np.arange(args.disease_number)
        entity_index = 1
    else:
        raise ValueError("Unsupported cold-start mode: {}".format(mode))

    kf = KFold(n_splits=k, shuffle=True, random_state=args.random_seed)
    X_train_all, X_test_all, Y_train_all, Y_test_all = [], [], [], []

    for train_entity_idx, test_entity_idx in kf.split(entities):
        test_entities = set(entities[test_entity_idx])
        mask = np.isin(X[:, entity_index], list(test_entities))
